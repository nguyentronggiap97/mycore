...
$(document).ready(function() {
     ...
     */
    cedu.uploader.showUploader = function(element) {
        // Update modal upload data
        $("#upload-data").val(JSON.stringify(element.data("upload")));
    ...
    cedu.uploader.getUploadedFiles = function() {
        ...
        var upload = JSON.parse($("#upload-data").val());
        ...
        $.ajax({
            ...
            success: function ( json ) {
                ...
                if(cedu.uploader.files.length) {
                    ...
                    $(".fm_file_selector ul").html("<div class='text-center text-danger' style='margin-top:40px; font-weight: 400;'>You do not have image files</div>");
                }
            }
        });
    }
    ...
    $(".btn_upload_image").on("click", function() {
        cedu.uploader.showUploader( $(this) );
    });
    ...
    cedu.dropzone = new Dropzone("#fm_dropzone", {
        ...
        init: function() {
            ...
            this.on('success', function(file) {
                ...
                cedu.uploader.getUploadedFiles();
                // Refresh upload list
                if (typeof cedu.uploads === "object" && typeof cedu.uploads.getUploadedFiles === "function") {
    ...
    $("body").on("click", "div.fm_file_selector .fm_file_sel", function() {
        ...
        var photo  = JSON.parse($(this).attr("upload"));
        var upload = JSON.parse($("#upload-data").val());
        var format = upload.format;

        console.log("[uploader] select file:", photo);
        console.log("[uploader] upload data:", upload);

        // Callback to process photo here
        if(upload.callback !== undefined) {
            var callback = new Function(upload.callback);
            var next = callback(photo);

            // Cancel section below
            if (next === false) {
                return $("#fm").modal('hide');
            }
        }

        if(format == "image") {
            $hinput = $("input[name=" + upload.name + "]");
            $hinput.val(photo.id);

            $hinput.next("a").addClass("hide");
            $hinput.next("a").next(".uploaded_image").removeClass("hide");
            $hinput.next("a").next(".uploaded_image").children("img").attr("src", photo.thumb);
        } else if(format == "file") {
            $hinput = $("input[name=" + upload.name + "]");
            $hinput.val(photo.id);
            ...
            $hinput.next("a").next(".uploaded_file").attr("href", photo.src);
            ...
            if(!upload_id_exists) {
                hiddenFIDs.push(photo.id);
            }
            $hinput.val(JSON.stringify(hiddenFIDs));
            var fileImage = "";
            if(photo.ext == "jpg" || photo.ext == "png" || photo.ext == "gif" || photo.ext == "jpeg") {
                fileImage = "<img src='" + photo.thumb + "' />";
            } else {
                fileImage = "<i class='fa fa-file-o'></i>";
            }
            $hinput.next("div.uploaded_files").append("<a class='uploaded_file2' upload_id='" + photo.id + "' target='_blank' href='" + photo.thumb + "'>" + fileImage + "<i title='Remove File' class='fa fa-times'></i></a>");
        }
        $("#fm").modal('hide');
    });
});